...
- [x] Frontend: code-split — extracted `ChatMessage.jsx` (react-markdown + remark-gfm + react-syntax-highlighter, the single heaviest slice of the bundle) and lazy-loaded it behind `React.lazy`/`Suspense`. Measured: main bundle 1,323.74 kB → 550.26 kB (gzip 448.55 kB → 178.37 kB), a verified ~60% cut to what has to parse/execute before first paint
- [x] Structured logging: added `logging_config.py` (level via `LOG_LEVEL`, quiets noisy httpx/chromadb loggers), converted all 23 `print()` calls across `main.py`, `llm.py`, `rag.py`, `design_service.py` to leveled `logger.info/warning/error` calls. Full 26-test suite re-verified green with `LOG_LEVEL=DEBUG` set
